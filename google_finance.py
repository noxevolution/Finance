...
class GoogleFinanceConfig (Model):
	ticker = CharField()
	start = DateField()
	end = DateField()
...
class GoogleFinanceResult (Model):
...
def FetchDataFromWeb (): 
	...
	print "\nFetching data ..."
	try:
		code = GoogleFinanceConfig.select().where(GoogleFinanceConfig.processing != True).get()
	except:
		...
		delete_quary = GoogleFinanceResult.delete().where(GoogleFinanceConfig.processing != True)
		delete_quary.execute()
		
		f=web.DataReader(code.ticker, 'google', code.start, code.end)
		for date, _value in f.iterrows():
			try:
				...
				record = GoogleFinanceResult.create(
...
if __name__ == '__main__':
	try:
		GoogleFinanceResult.create_table()
	except Exception, e:
		print "\nException: ", e
	
	try:
		GoogleFinanceConfig.create_table()
	except Exception, e:
		print "\nException: ", e
